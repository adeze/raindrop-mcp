...
describe("remove_duplicates tool", () => {
  let mockService: any;
  const removeDuplicates = cleanupTools.find(t => t.name === "remove_duplicates")!;
  ...
  it("returns 'No duplicates found' if global count is 0", async () => {
    ...
    const result = await removeDuplicates.handler({ dryRun: true }, { raindropService: mockService } as any);

    const firstContent = result.content[0] as { type: "text"; text: string };
    expect(firstContent.text).toContain("No duplicates found");
    expect(mockService.getBookmarks).toHaveBeenCalledWith(expect.objectContaining({ duplicates: true, perPage: 1 }));
  });
  ...
  it("performs dry run and reports counts per collection", async () => {
    ...
    // 2. Get collections
    mockService.getCollections.mockResolvedValueOnce([{ _id: 123, title: "Test Collection" }]);
    // 3. Per-collection search (Unsorted)
    mockService.getBookmarks.mockResolvedValueOnce({ count: 0, items: [] });
    // 4. Per-collection search (Test Collection)
    mockService.getBookmarks.mockResolvedValueOnce({ count: 2, items: [{ _id: 1 }, { _id: 2 }] });
  ...
  it("deletes duplicates in batches of 50 and handles Fail Fast", async () => {
    ...
    mockService.getBookmarks.mockResolvedValueOnce({ count: 0, items: [] });
    // 4. Per-collection search (Test Collection)
    mockService.getBookmarks.mockResolvedValueOnce({ count: 1, items: [{ _id: 99 }] });
    ...
    expect(mockService.batchDeleteBookmarksInCollection).toHaveBeenCalledWith(123, [99]);
  });
  ...
  it("successfully deletes duplicates across multiple collections", async () => {
    ...
    mockService.getBookmarks.mockResolvedValueOnce({ count: 2, items: [] });
    // 2. Get collections
    mockService.getCollections.mockResolvedValueOnce([{ _id: 123, title: "Col A" }, { _id: 456, title: "Col B" }]);
    // 3. Unsorted
    mockService.getBookmarks.mockResolvedValueOnce({ count: 0, items: [] });
    // 4. Col A
    mockService.getBookmarks.mockResolvedValueOnce({ count: 1, items: [{ _id: 1 }] });
    // 5. Col B
    mockService.getBookmarks.mockResolvedValueOnce({ count: 1, items: [{ _id: 2 }] });
    // 6. Delete A
